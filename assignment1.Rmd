...
ggplot(ces4, aes(x=as.numeric(low_birth_weight))) +
  geom_histogram(bins = 30, fill = "deepskyblue4", color = "gray50") +
...
**Interpretation: As we can see in the results above we can interpret that:**
- Beta Cero is 3.8, meaning that on average the weight of the low_birth variable is 3.08 Percent (Making PM2.5 = 0). 
- Beta One which is 0.118, this means that on average, for every one unit increase of PM2.5 the percent of low birth weight increases by 0.118 %. 
- The coefficients for the slope (Beta One) tell us that we are 95% confident that our slope or Beta One will fall between `r olm_robust$conf.low` and `r olm_robust$conf.high`.** INCOMPLETE
...
**Interpretation: For the indicator interpretation we can say that on average, for every increase of the indicator variable the percetage of low_birth_weight will increase 0.129 percent. In other words, if the LinguisticIsolation is above the average > than `r x` then the effect in low_birth_weight will be 0.13 percent.**
